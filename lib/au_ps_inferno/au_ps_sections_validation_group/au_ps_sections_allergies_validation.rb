...
module AUPSTestKit
  ...
  class AUPSSectionsAllergiesValidation < SectionTest
    title 'Validate ALLERGIES AND ADVERSE REACTIONS DOCUMENT Section References and Resources'
    description 'Validates that the ALLERGIES AND ADVERSE REACTIONS DOCUMENT section in the Composition resource contains valid references that resolve to expected resource types in the bundle, and that each referenced resource conforms to its specified FHIR profile(s).'
    id :au_ps_sections_allergies_validation
    ...
    run do
      ...
      read_and_save_data
      validate_section_resources('ALLERGIES_AND_ADVERSE_REACTIONS_DOCUMENT')
    end
  end
end
