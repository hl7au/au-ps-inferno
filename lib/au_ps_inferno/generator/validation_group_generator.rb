...
require_relative 'validation_group_tests'
require_relative 'test_file_generator'

class Generator
  # Generates the au_ps_validation_group and its test files for a given version.
  #
  # One bundle-is-valid test (BundleIsValidClass) plus six BasicTest tests (must-support,
  # composition sections). Validates a user-provided Bundle; no FHIR client. Uses
  # validation_group.rb.erb and retrieve_bundle_test.rb.erb.
  class ValidationGroupGenerator
    GROUP_NAME = 'au_ps_validation_group'
    GROUP_TITLE = 'AU PS Bundle Validation'
    GROUP_DESCRIPTION = 'Verify that an AU PS Bundle is valid and contains required must support elements.'
    ...
    def versioned_group_class
      base = 'AUPSValidationGroup'
      @version_suffix.empty? ? base : "#{base}#{@version_suffix}"
    end

    def generate_group
      config = {
        template_file_path: 'validation_group.rb.erb',
        output_file_path: 'au_ps_validation_group.rb',
    ...
    def group_attributes
      {
        ...
        group_class_comment: 'Verify that the AU PS Bundle is valid',
        tests: @test_entities
      }
    end
    ...
    def run_code(spec, metadata)
      ...
      when 'au_ps_composition_recommended_sections'
        "read_composition_sections_info(#{metadata.recommended_sections_data_codes}, #{metadata.return_normalized_sections_data})"
      when 'au_ps_composition_optional_sections'
        "read_composition_sections_info(#{metadata.optional_sections_data_codes}, #{metadata.return_normalized_sections_data})"
