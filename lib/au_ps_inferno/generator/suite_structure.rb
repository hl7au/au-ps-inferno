...
require_relative 'suite_structure_shared_groups'
require_relative 'suite_structure_high_order_configs'
...
class Generator
  ...
  #
  # To add a new high-order group: add an entry to HIGH_ORDER_GROUP_CONFIGS (in suite_structure_high_order_configs.rb).
  # To add a new shared group: add to SHARED_GROUP_DEFINITIONS (in suite_structure_shared_groups.rb).
  module SuiteStructure
    # Builds the full HIGH_ORDER_GROUPS structure expected by Generator:
    # array of hashes with :name, :description, :groups.
    ...
    def self.expand_high_order_groups
      HIGH_ORDER_GROUP_CONFIGS.map { |config| build_high_order_group(config) }
    end

    def self.build_high_order_group(config)
      groups = SHARED_GROUP_DEFINITIONS.map { |group_def| build_group(group_def, config) }
    ...
    def self.build_group(group_def, config)
      tests = group_def[:tests].map { |t| expand_test(t, config) }
    ...
    def self.expand_test(test_spec, config)
      return bundle_validation_test_config(config) if test_spec == BUNDLE_VALIDATION_PLACEHOLDER

      { id: test_spec[:id] }
    end

    def self.bundle_validation_test_config(config)
      {
        id: :bundle_valid,
