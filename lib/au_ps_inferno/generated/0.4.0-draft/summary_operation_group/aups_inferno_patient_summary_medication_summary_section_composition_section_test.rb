...
module AUPSTestKit
  class AUPSInfernoPatientSummaryMedicationSummarySectionCompositionSectionTest < Inferno::Test
    title 'Validate Patient Summary Medication Summary Section'
    description 'This test verifies that the Patient Summary Medication Summary Section within the Composition entry of a $summary Bundle is correctly structured. It extracts the references listed in the section, checks that the corresponding resources exist in the Bundle, and ensures they conform to the expected resource type and profile requirements.'
    id :au_ps_patient_summary_medication_summary_section_composition_section_test
    ...
    run do
      ...
      target_resources_and_profiles = 'MedicationStatement::http://hl7.org.au/fhir/ps/StructureDefinition/au-ps-medicationstatement'.split(';').map do |segment|
