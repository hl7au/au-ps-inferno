...
module AUPSTestKit
  class AUPSInfernoPatientSummaryPatientStorySectionCompositionSectionTest < Inferno::Test
    title 'Validate Patient Summary Patient Story Section'
    description 'This test verifies that the Patient Summary Patient Story Section within the Composition entry of a $summary Bundle is correctly structured. It extracts the references listed in the section, checks that the corresponding resources exist in the Bundle, and ensures they conform to the expected resource type and profile requirements.'
    id :au_ps_patient_summary_patient_story_section_composition_section_test
    ...
    run do
      ...
      current_section = composition_resource.section.find { |s| s.code.coding.first.code == '81338-6' }
      ...
      target_resources_and_profiles = ''.split(';').map do |segment|
