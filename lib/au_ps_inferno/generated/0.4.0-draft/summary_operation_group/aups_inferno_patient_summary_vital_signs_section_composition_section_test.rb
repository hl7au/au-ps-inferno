...
module AUPSTestKit
  class AUPSInfernoPatientSummaryVitalSignsSectionCompositionSectionTest < Inferno::Test
    title 'Validate Patient Summary Vital Signs Section'
    description 'This test verifies that the Patient Summary Vital Signs Section within the Composition entry of a $summary Bundle is correctly structured. It extracts the references listed in the section, checks that the corresponding resources exist in the Bundle, and ensures they conform to the expected resource type and profile requirements.'
    id :au_ps_patient_summary_vital_signs_section_composition_section_test
    ...
    run do
      ...
      current_section = composition_resource.section.find { |s| s.code.coding.first.code == '8716-3' }
      ...
      target_resources_and_profiles = 'Observation::http://hl7.org/fhir/StructureDefinition/vitalsigns'.split(';').map do |segment|
