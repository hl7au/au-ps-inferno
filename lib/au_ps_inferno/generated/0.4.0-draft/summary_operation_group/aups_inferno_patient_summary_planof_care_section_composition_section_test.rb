...
module AUPSTestKit
  class AUPSInfernoPatientSummaryPlanofCareSectionCompositionSectionTest < Inferno::Test
    title 'Validate Patient Summary Plan of Care Section'
    description 'This test verifies that the Patient Summary Plan of Care Section within the Composition entry of a $summary Bundle is correctly structured. It extracts the references listed in the section, checks that the corresponding resources exist in the Bundle, and ensures they conform to the expected resource type and profile requirements.'
    id :au_ps_patient_summary_plan_of_care_section_composition_section_test
    ...
    run do
      ...
      current_section = composition_resource.section.find { |s| s.code.coding.first.code == '18776-5' }
      ...
      target_resources_and_profiles = 'CarePlan::http://hl7.org/fhir/StructureDefinition/CarePlan;ImmunizationRecommendation::http://hl7.org/fhir/StructureDefinition/ImmunizationRecommendation'.split(';').map do |segment|
