...
module AUPSTestKit
  class AUPSInfernoIPSPlanofCareSectionCompositionSectionTest < Inferno::Test
    title 'Validate IPS Plan of Care Section'
    description 'This test verifies that the IPS Plan of Care Section within the Composition entry of a $summary Bundle is correctly structured. It extracts the references listed in the section, checks that the corresponding resources exist in the Bundle, and ensures they conform to the expected resource type and profile requirements.'
    id :au_ps_ips_plan_of_care_section_composition_section_test
    ...
    run do
      ...
      target_resources_and_profiles = 'CarePlan::http://hl7.org/fhir/StructureDefinition/CarePlan'.split(';').map do |segment|
