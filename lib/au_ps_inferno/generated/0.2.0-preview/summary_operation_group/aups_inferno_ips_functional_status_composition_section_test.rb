...
module AUPSTestKit
  class AUPSInfernoIPSFunctionalStatusCompositionSectionTest < Inferno::Test
    title 'Validate IPS Functional Status'
    description 'This test verifies that the IPS Functional Status within the Composition entry of a $summary Bundle is correctly structured. It extracts the references listed in the section, checks that the corresponding resources exist in the Bundle, and ensures they conform to the expected resource type and profile requirements.'
    id :au_ps_ips_functional_status_composition_section_test
    ...
    run do
      ...
      target_resources_and_profiles = 'Condition::http://hl7.org/fhir/uv/ips/StructureDefinition/Condition-uv-ips;ClinicalImpression::http://hl7.org/fhir/StructureDefinition/ClinicalImpression'.split(';').map do |segment|
