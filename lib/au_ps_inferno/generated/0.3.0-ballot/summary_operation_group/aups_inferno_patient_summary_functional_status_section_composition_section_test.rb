...
module AUPSTestKit
  class AUPSInfernoPatientSummaryFunctionalStatusSectionCompositionSectionTest < Inferno::Test
    title 'Validate Patient Summary Functional Status Section'
    description 'This test verifies that the Patient Summary Functional Status Section within the Composition entry of a $summary Bundle is correctly structured. It extracts the references listed in the section, checks that the corresponding resources exist in the Bundle, and ensures they conform to the expected resource type and profile requirements.'
    id :au_ps_patient_summary_functional_status_section_composition_section_test
    ...
    run do
      ...
      current_section = composition_resource.section.find { |s| s.code.coding.first.code == '47420-5' }
      ...
      target_resources_and_profiles = 'Condition::http://hl7.org.au/fhir/ps/StructureDefinition/au-ps-condition;ClinicalImpression::http://hl7.org/fhir/StructureDefinition/ClinicalImpression'.split(';').map do |segment|
